...
def main():
    ...
    for i in range(len(predict)):
        print("class: {:10}   prob: {:.3}".format(class_indict[str(i)],
                                                  predict[i].numpy()))
        if predict[i].numpy() > tempt:  # 记录最大概率的类别
    ...
    # image = Image.fromarray(cv2.cvtColor(new_image, cv2.COLOR_BGR2RGB))  # opencv转PIL

    mp = Mphand(new_image)
    mp.Select_Mode(class_text, mode=False)  # mode=True(default)：姿态估计， False：手势识别
